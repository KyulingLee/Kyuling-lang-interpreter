...
void convertToInternalCode(char *fname)
{
    ...
    fileOpen(fname);    
    while (token=nextLineToken(), token.kind != EofProg) 
    {
        if (token.kind == Func) 
        {    
            token = nextToken(); 
...
void convertBlockSet()
{
    ...
    //이건 end행 번호로 바꿈
    backPatch(patch_line, getLineNo());      
}
    ...
    for (;;) 
    {
        ...
        {      
            ...
            case Ident:     
                ...
                if (isLocalName(tmpTb.name, varId)) 
                {
                    setCode(Lvar, tblNbr);
                }
                else                                 
            ...
            case IntNum: 
            case DblNum:                        
                setCode(token.kind, setLITERAL(token.dblVal));
                break;
...
void setaryLength()
{
    ...
    token = checkNextToken(nextToken(), ']');
...
{
    *SHORT_POINT(intercode[line] + 1) = (short)n;
}
